...
//    ha.hora = hora;
  ...
  Future<String> _getDropDownData() async {
    var res = await http.get('https://www.accio.com.br:447/api/v1/agendamentos/datasAgendamentos/datas/?access_token=' + token.access_token.toString());
    print(res.body);
...
//    List<DataAgendamento> teste = list.map((i)=> DataAgendamento.fromJson(i)).toList();
  ...
  Future<String> _getDropDownHora(int id) async {
    var res = await http.get(Uri.encodeFull('https://www.accio.com.br:447/api/v1/agendamentos/horasAgendamentos/horarios/nao-reservadas/' + id.toString() + '/?access_token=' + token.access_token.toString()));
    return res.body;
  }
